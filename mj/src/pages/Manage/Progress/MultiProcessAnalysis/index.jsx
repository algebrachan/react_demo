...
let point_list = [];
const temp_point_obj = {};

function MultiProcessAnalysis() {
  ...
  const initChart = () => {
    if (!myChartRef.current) {
      ...
      myChartRef.current.getZr().on("mousedown", (params) => {
        const { offsetX, offsetY, target } = params;
        if (target !== void 0 && typeof target.parent.id === "string") {
          console.log("mousedown", params);
          temp_point_obj["start_point"] = [offsetX, offsetY];
        }
      });
      myChartRef.current.getZr().on("mouseup", (params) => {
        const { offsetX, offsetY, target } = params;
        if (target !== void 0 && typeof target.parent.id === "string") {
          console.log("mouseup", params);
          let idx = Number(target.parent.id.split("_")[1]);
          // 相对位置
          let relative_x = offsetX - temp_point_obj["start_point"][0];
          let relative_y = offsetY - temp_point_obj["start_point"][1];
          point_list[idx].x += relative_x;
          point_list[idx].y += relative_y;
        }
      });
    }
    // 获取 chart_data 的key
  ...
  useEffect(() => {
    if (myChartRef.current) {
      ...
      myChartRef.current.getZr().on("click", function (event) {
        if (isRecord) {
          setIsRecord(!isRecord);
          var offsetX = event.offsetX;
          var offsetY = event.offsetY;
          setIsModalOpen(true);
          setModalPoint([offsetX, offsetY]);
        }
      });
    }
  }, [isRecord]);
  ...
  return (
    <div>
      <MyBreadcrumb items={["熔融机过程分析", "多过程分析"]} />
            ...
            <Panel defaultSize={72} minSize={30}>
              <div style={{ width: "100%", height: 540, position: "relative" }}>
                <Space style={{ position: "absolute", top: 5, right: 5 }}>
                  ...
                  <Button onClick={() => setIsClearOpen(true)}>清除标记</Button>
                ...
                <GeneralCard name="多过程分析图">
                  <Spin spinning={chart_spin}>
                    <div
                      style={{ padding: 16, width: "100%", height: 500 }}
                      ref={lineRef}
                    ></div>
                  </Spin>
                </GeneralCard>
                    ...
                    <Form.Item name="param" {...ComputeFormCol(0)}>
                      <Radio.Group style={{ width: "100%" }}>
                        {ckb_param_list.map((x, _) => (
                          <Radio value={x} key={_} className="check_box_item">
                            {x}
                          </Radio>
                        ))}
                      </Radio.Group>
...
export default MultiProcessAnalysis;
