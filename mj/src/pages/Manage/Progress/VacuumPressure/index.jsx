...
function VacuumPressure() {
  ...
  const [chart_data4, setChartData4] = useState({});
  ...
  const query = (val) => {
    getPprId(
      ...
      (res) => {
        ...
        if (code === 0 && data) {
          const { ppr_id } = data;
          let temp_ppr_id = ppr_id.map((e) => e.id);
          setWorkList(temp_ppr_id);
          setCkbWorkList(temp_ppr_id);
        } else {
          setWorkList([]);
          setCkbWorkList([]);
        }
    ...
    getFeatureStatLine(
      ...
      (res) => {
        ...
        if (code === 0 && data) {
          setChartData1(data);
          let val = {};
          let arr = Object.keys(data).map((e) => data[e][data[e].length - 1].x);
          if (arr.length > 0) {
            let start = 0;
          ...
          Object.keys(data).forEach((e) => (val[e] = data[e].slice(500, 1001)));
          setChartData4(val);
        } else {
          setChartData1({});
        }
  ...
  return (
    <div>
      <MyBreadcrumb items={["熔融机过程分析", "批次真空压力管控"]} />
          ...
          <PanelGroup direction="horizontal">
            ...
            <Panel defaultSize={88} minSize={70}>
              <Row gutter={[10, 12]}>
                <Col span={16}>
                  <Spin spinning={load1}>
                    <GeneralCard name="模具真空压力">
                      <div style={{ height: 440, width: "100%", padding: 10 }}>
                ...
                <Col span={8} style={{ position: "relative" }}>
                  ...
                  <GeneralCard name="批次异常监控">
                    <div
                      style={{ height: 440, width: "100%", padding: 10 }}
                      className="of"
                    >
                      <Row gutter={[0, 10]}>
                        <Col span={24}>
                ...
                <Col span={8}>
                  <Spin spinning={load2}>
                    <GeneralCard name="起弧阶段">
                      <div style={{ height: 360, width: "100%", padding: 5 }}>
                ...
                <Col span={8} style={{ position: "relative" }}>
                  <Spin spinning={load2}>
                    ...
                    <GeneralCard name="起弧阶段通道管控">
                      <div
                        style={{ height: 360, width: "100%", padding: 10 }}
                        className="of"
                      >
