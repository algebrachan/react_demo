...
import './gp_B01.less';
import {file_management_process_audit,upload_file_process_audit,download_file_download_file,download_file_download_file_delete} from '../../../../apis/qms_router'
...
export default function GpB01({ planData }) {
  ...
  const fetchFileManagementData = () => {
    if (planData && planData.id) {
      file_management_process_audit({ id: planData.id }, res => {
  ...
  // 当planData变化时，调用file_management_process_audit接口
  useEffect(() => {
  ...
  const customUpload = async (options, deptIndex) => {
    ...
    try {
      ...
      // 调用upload_file接口
      upload_file_process_audit({id: department.id},formData, 
        (res) => {
          if (res.data && res.data.code === 200) {
  ...
  const downloadFile1 = (deptIndex, fileIndex) => {
    ...
    download_file_download_file({ path: file.path }, res => {
  ...
  const deleteFile = (deptIndex, fileIndex) => {
    ...
    Modal.confirm({
      ...
      cancelText: '取消',
      onOk: () => {
        download_file_download_file_delete({ path: file.path,id: departments[deptIndex].id}, res => {
          if (res.data && res.data.code === 200) {
