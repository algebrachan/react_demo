...
import './gp_E01.less';
import {item_tracking_post,item_tracking_delete_process_audit,item_tracking_put_process_audit,audit_plan_put,nick_name,item_tracking_get,upload_file_process_audit,download_file_download_file,download_file_download_file_delete,item_tracking_put} from '../../../../apis/qms_router'
...
const handleSave = async () => {
  try {
    ...
    audit_plan_put(formattedValues,res=>{
  ...
  const fetchTableData = () => {
    return new Promise((resolve) => {
      item_tracking_get({plan_id: planData.id}, (res) => {
        let latestData = [];
...
useEffect(() => {
    form.setFieldsValue({
      prepared_item: planData.prepared_item,
      prepared_date_item: planData.prepared_date_item ? dayjs(planData.prepared_date_item) : null,
      countersign_item: planData.countersign_item,
      countersign_date_item: planData.countersign_date_item ? dayjs(planData.countersign_date_item) : null,
      final_approved_item: planData.final_approved_item,
      final_approved_date_item: planData.final_approved_date_item ? dayjs(planData.final_approved_date_item) : null
    });
})
  ...
  const saveRowData = (rowData, step) => {
    const params = {
      ...
      element: rowData.element || 0,
      non_conform_desc: rowData.non_conform_desc || '',
      cause_analysis: rowData.cause_analysis || '',
      rectification_measures: rowData.rectification_measures || '',
      charge_person: rowData.charge_person || '',
    ...
    // 如果是已存在的项目，使用review_plan_put请求更新
    if (rowData.id) {
      params.id = rowData.id;
      item_tracking_put_process_audit(params, (res) => {
        if (res.data && res.data.code === 200) {
      ...
      // 如果是新项目，使用item_tracking_post请求创建
      item_tracking_post(params, (res) => {
        if (res.data && res.data.code === 200) {
          message.success('保存成功');
          fetchTableData(); // 刷新表格
        } else {
  ...
  const addNewItem = () => {
    ...
    item_tracking_post(newItem, (res) => {
      if (res.data && res.data.code === 200) {    
        fetchTableData(); // 刷新表格 
  ...
  const removeItem = (itemId) => {
    ...
    // 如果是服务器数据，调用API删除
    item_tracking_delete_process_audit({id: itemId}, (res) => {
      if (res.data && res.data.code === 200) {
        fetchTableData(); // 刷新表格
        message.success("改进项目已删除");
      } else {
  ...
  const handleReturn =  (record) => {
      const params = {
        ...
        non_conform_desc: record.non_conform_desc,
        cause_analysis: record.cause_analysis,
        rectification_measures: record.rectification_measures,
        charge_person: record.charge_person,
 ...
 item_tracking_put_process_audit(params,res=>{
  ...
  const handleFileDelete = (filePath) => {
    ...
    download_file_download_file_delete({
      id: currentEvidenceRow.id,
      typeof: 2,
      path: filePath
    }, (res) => {
  ...
  const columns = [
    ...
    // 橙色体系工程师列
    {
      ...
      dataIndex: 'element',
      width: 100,
      className: 'orange-cell orange-column',
      render: (text, record) => (
        <Input
          value={text}
          onChange={(e) => updateTableData(record.id, 'element', e.target.value)}
          placeholder="请输入要素编号"
    ...
    {
      title: '不符合项描述',
      dataIndex: 'non_conform_desc',
      width: 200,
      className: 'orange-cell orange-column',
      render: (text, record) => (
        <TextArea
          value={text}
          onChange={(e) => updateTableData(record.id, 'non_conform_desc', e.target.value)}
          placeholder="请输入不符合项描述"
          rows={2}
          disabled={!canOperateStep(record, 0)}
        />
      )
    ...
    // 绿色负责人列
    {
      ...
      width: 200,
      className: 'green-cell green-column',
      render: (text, record) => (
        <TextArea
          value={text}
          onChange={(e) => updateTableData(record.id, 'cause_analysis', e.target.value)}
          placeholder="请输入原因分析"
          rows={2}
    ...
    },
    {
      ...
      render: (text, record) => (
        <TextArea
          value={text}
          onChange={(e) => updateTableData(record.id, 'rectification_measures', e.target.value)}
          placeholder="请输入整改措施"
          rows={2}
          disabled={!canOperateStep(record, 1)}
        />
    ...
    },
    {
      ...
      render: (text, record) => (
        <Input
          value={text}
          onChange={(e) => updateTableData(record.id, 'charge_person', e.target.value)}
          placeholder="请输入担当人"
          disabled={!canOperateStep(record, 1)}
        />
      )
    },
    {
      ...
      render: (text, record) => (
        <DatePicker
          ...
          onChange={(date, dateString) => updateTableData(record.id, 'finish_date', dateString)}
          placeholder="选择日期"
          disabled={!canOperateStep(record, 1)}
          style={{ width: '100%' }}
        />
    ...
    },
    {
      ...
      render: (text, record) => (
        <DatePicker
          value={text ? dayjs(text) : null}
          onChange={(date, dateString) => updateTableData(record.id, 'close_date', dateString)}
          placeholder="选择日期"
          disabled={!canOperateStep(record, 1)}
          style={{ width: '100%' }}
        />
    ...
    // 蓝色体系工程师列
    {
      ...
      dataIndex: 'is_closed',
      width: 80,
      className: 'blue-cell blue-column',
      render: (text, record) => (
        <Select
          value={text}
          onChange={(value) => updateTableData(record.id, 'is_closed', value)}
          placeholder="选择"
    ...
    },
    {
      ...
      render: (text, record) => (
        <Select
          value={text}
          onChange={(value) => updateTableData(record.id, 'is_invalid', value)}
          placeholder="选择"
          disabled={!canOperateStep(record, 2)}
          style={{ width: '100%' }}
        >
    ...
    },
    {
      ...
      render: (_, record) => (
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          ...
          {/* 体系工程师退回按钮 */}
          {isSystemEngineer && record.step > 0 && (
            <Button
              type="default"
              size="small"
  ...
  <div style={{ textAlign: 'center' }}>
    <Button type="primary" onClick={handleSave} style={{ width: '120px' }}>
      保存
    </Button>
  </div>
