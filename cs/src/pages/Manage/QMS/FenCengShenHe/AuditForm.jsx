...
import { get_audit_content, audit_content, non_conform, get_non_conform, non_conform_put, nick_name, examine_nick_name } from "../../../../apis/qms_router";
const { Option } = Select;
...
function AuditForm({ record: propRecord, onClose, isModal = false }) {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (text, record, index) => {
                // 如果是新增的行，显示输入框；否则显示原文本
                if (record.isNew) {
                    return (
                        <Input
                            value={text}
                            onChange={(e) => handleTableChange(index, '检查内容', e.target.value)}
                            placeholder="请输入检查内容"
                            style={{
                                color: xuhao.includes(record.序号) ? 'red' : undefined
                            }}
                        />
                    );
                } else {
            ...
            render: (text, record, index) => {
                ...
                return (
                    <div>
                        <Input
                            value={text}
                            onChange={(e) => handleTableChange(index, '问题描述', e.target.value)}
    ...
    const handleSubmit = () => {
        form.validateFields().then(values => {
            // 校验N/C必填问题描述
            const ncRows = tableData.filter(row => row.结果评价 === 'N/C');
            const missingDescriptions = ncRows.filter(row => !row.问题描述 || row.问题描述.trim() === '');
            ...
            const submitData = {
                ...
                "area": values.审核区域,
                "reviewer": values.审核人,
                "review_date": values.审核日期 ? dayjs(values.审核日期).format(dateFormat) : '',
                "confirm": values.陪同人,
                department: record.department,
                workshop: record.workshop,
                data: tableData
    ...
    const handleAddRow = () => {
        const newRow = {
            序号: tableData.length > 0 ? Math.max(...tableData.map(item => item.序号 || 0)) + 1 : 1,
            检查内容: '',
            结果评价: '',
            问题描述: '',
            isNew: true // 标记为新增行
